...
public class table 
{
	...
	int d=1100;
	int i;
	Color colorT=Color.blue;
	Color colors[]={colorT,colorT,colorT,
			        colorT,colorT,colorT, };

	Point3D pt1=new Point3D(),pt2=new Point3D(),pt3=new Point3D();
	
	public void buildshape1(double x, double y ,double z,double dz )
	{
		xReal[0]=x;        yReal[0]=y;         zReal[0]=z; 
		xReal[1]=x+40;    yReal[1]=y;         zReal[1]=z; 
		xReal[2]=x+40;    yReal[2]=y+40;     zReal[2]=z; 
		xReal[3]=x;        yReal[3]=y+40;     zReal[3]=z; 

		xReal[4]=x;        yReal[4]=y;         zReal[4]=z+dz; 
		xReal[5]=x+40;    yReal[5]=y;         zReal[5]=z+dz; 
		xReal[6]=x+40;    yReal[6]=y+40;     zReal[6]=z+dz; 
		xReal[7]=x;        yReal[7]=y+40;    zReal[7]=z+dz; 
...
//    for (i=2;i<numOfPoint;i++)
//    {
//	    xGuf[i][0]=xReal[i-2];                yGuf[2][0]=yReal[i-2];                zGuf[2][0]=zReal[i-2];  
//		xGuf[i][1]=xReal[(i-2)+numOfPoint];   yGuf[2][1]=yReal[(i-2)+numOfPoint];   zGuf[2][1]=zReal[(i-2)+numOfPoint]; 
//		xGuf[i][2]=xReal[(i-2)+numOfPoint+1]; yGuf[2][2]=yReal[(i-2)+numOfPoint+1]; zGuf[2][2]=zReal[(i-2)+numOfPoint+1];
//		xGuf[i][3]=xReal[i-1];                yGuf[2][3]=yReal[i-1];                zGuf[2][3]=zReal[i-1];  
//    }	
//		
	...
	public void convertAndShow(Graphics page,Color color,boolean upT, boolean rightT, 
		                                            	boolean downT, boolean leftT,
		                                            	boolean rama,
		                                            	Point3D magoz, Point3D plight)
	{
		...
		for(int i =0;i<2;i++)
		{
			...
			if ( check(xInt[0],yInt[0],xInt[1],yInt[1],xInt[2],yInt[2]) )
			{
				...
				page.setColor(colorToShow(plight,pt1,pt2,pt3,color));
				page.fillPolygon(xInt,yInt, numOfPoint);
				
				page.setColor(colorToShow(plight,pt1,pt2,pt3,Color.blue));
		...
		for(int i =2;i<numOfPoint+2;i++)
		{
			...
			if ((i==2&&upT)||(i==3&&rightT)||(i==4&&downT)||(i==5&&leftT)||rama)
			{
				convert3D(xGuf[i],yGuf[i],zGuf[i],4,magoz);
				...
				if ( check(xInt[0],yInt[0],xInt[1],yInt[1],xInt[2],yInt[2]) )
				{
					...
					page.setColor(colorToShow(plight,pt1,pt2,pt3,colors[i]));
...
//		for(int i =2;i<numOfPoint+2;i++)
//		{
//			convert3D(xGuf[i],yGuf[i],zGuf[i],4);
//		
//			if ( check(xInt[0],yInt[0],xInt[1],yInt[1],xInt[2],yInt[2]) )
//			{
//		    page.setColor(colors[i]);
...
//		}
	...
	public void convertAndShow2(Graphics page,Color color)
	{
